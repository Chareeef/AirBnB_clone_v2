...
def do_clean(number=0):
    ...
    cp_local = f'cp -f $(ls -c1 versions/ | head -{n} | '
    cp_local += 'awk \'{print "versions/" $1}\') ./'

    local(cp_local)
    local('rm -f versions/*')
    local('mv *.tgz versions/')

    cp_remote = f'cp -rf $(ls -c1 /data/web_static/releases/ | head -{n} | '
    cp_remote += 'awk \'{print "/data/web_static/releases/" $1}\') /tmp/'

    run(cp_remote)
    run('rm -rf /data/web_static/releases/*')
    run('mv /tmp/web_static_* /data/web_static/releases/')
